...
from utils.model import CNNModel, generate_caption_beam_search , generate_caption
import os
...
for image_file in os.listdir(config['test_data_path']):
	...
	if(image_file.split('.')[1]=='jpg' or image_file.split('.')[1]=='jpeg'):
		...
		# _ = ax.set_title("BEAM Search with k={}\n{}".format(config['beam_search_k'],caption),fontdict={'fontsize': '16','fontweight' : '40'})
		# plt.savefig(config['test_data_path']+'output--'+image_file)
		
		_ = ax.set_title("Argmax\n{}".format(caption),fontdict={'fontsize': '16','fontweight' : '40'})
		plt.savefig(config['test_data_path']+'output--'+image_file)